...
function TableBuilder({ showHeader = true, data, readOnly, onChange }) {
  const {
    mouse,
    selectedCells,
    memoizedPlaceCells,
  ...
  return (
    <div className="mergeable-table-component" style={{ zIndex: 200 }}>
            ...
            return (
              <tr key={i}>
                {columns.map((_cell, j) => {
                  ...
                  const cell = data.elements.find(m => m.row === i && m.col === j) || {};
                  ...
                  return (
                    ...
                    >
                      <td
                        ...
                        {...{ rowSpan, colSpan }}
                        onMouseOver={
                          mouse ? onCellMouseOver(i + rowSpan - 1, j + colSpan - 1) : undefined
                        }
...
export default function({ config, defaultConfig, onChange, ...rest }) {
  if (config) {
    return <TableBuilder {...rest} data={config} onChange={onChange} />;
  }
  ...
  return <TableBuilder {...rest} data={_data} onChange={_onChange} />;
}
